 ...
 */
export function convertMilliseconds(milliseconds: number): { d: number, h: number, m: number, s: number, total: { h: number, m: number, s: number }} {
    const total_seconds = parseInt(Math.floor(milliseconds / 1000).toString(10), 10);
    const total_minutes = parseInt(Math.floor(total_seconds / 60).toString(10), 10);
    const total_hours = parseInt(Math.floor(total_minutes / 60).toString(10), 10);
    ...
    return { d: days, h: hours, m: minutes, s: seconds, total: { h: total_hours, m: total_minutes, s: total_seconds} };
}
